...
def get_update(start_year: int = _default_start_year,
    ...
    if start_year > _date.today().nfl_year:
        raise ValueError("update year greater than current year")
    dfs: Dict[str, pd.DataFrame] = {}
    headers: List[str] = []
    url: str = ''
    for dataset in datasets:
        ...
        """for y in range(start_year, _date.today().nfl_year + 1):
            ...
            dfs[dataset[0] + '_' + str(y)] = _client.submit(get_url_data,
                                                            url=url,
                                                            headers=headers)
            sleep(randrange(3, 6) + random())"""
        url = dataset[1].format(str(start_year))
        dfs[dataset[0] + '_' + str(start_year)] = _client.submit(
            get_url_data,
            url=url,
            headers=headers)
        sleep(randrange(3, 6) + random())
